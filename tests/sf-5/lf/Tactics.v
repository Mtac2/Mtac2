...
Theorem f_equal : forall (A B : Type) (f: A -> B) (x y: A),
  x = y -> f x = f y.
MProof. intros A B f x y eq. rewrite eq.  reflexivity.  Qed.
...
Theorem S_inj : forall (n m : nat) (b : bool),
     ...
     beq_nat n m = b.
MProof.
  intros n m b H. simpl_in H. apply H.  Qed.
...
MProof.
  intros n eq H.
  symmetry_in H. apply_in eq H. symmetry_in H.
  apply H.  Qed.
...
MProof.
  intros n m. elim n &> [i:~~ | \n'].
  - (* n = O *) simpl. destruct m &> [i: ~~ | \m'] &> intro eq.
    + (* m = O *) reflexivity.
    + (* m = S m' *) inversion eq.
  - (* n = S n' *) destruct m &> [i:~~ | \m'] &> intro IH &> intro eq.
    + (* m = O *) inversion eq.
    + (* m = S m' *) apply f_equal.
...
MProof.
  intros n. elim n &> [i: ~~| \n' IH].
  - (* n = O *) simpl. destructn 0 &> [i:~~| \m'] &> intro eq.
    + (* m = O *) reflexivity.
    + (* m = S m' *) inversion eq.
...
(** At this point, since we are in the second branch of the [destruct
    ...
    [IHn'] gives us exactly what we need to finish the proof. *)
      ...
      apply IH. inversion eq. reflexivity. Qed.
...
Theorem double_injective_take2_FAILED : forall n m,
     ...
     n = m.
MProof.
  intros n m. elim m &> [i:~~ | \m'].
  - (* m = O *) simpl. destruct n &> [i:~~ | \n'] &> intro eq.
    + (* n = O *) reflexivity.
    + (* n = S n' *) inversion eq.
  - (* m = S m' *) destruct n &> [i:~~| \n'] &> intro IH &> intro eq.
    + (* n = O *) inversion eq.
...
MProof.
  ...
  - (* beq_nat n 3 = true *) simpl. reflexivity.
  - (* beq_nat n 3 = false *)
    simpl.
    destruct (beq_nat _ _).
...
Theorem sillyfun1_odd_FAILED : forall (n : nat),
     ...
     oddb n = true.
MProof.
  intros n eq. unfold_in sillyfun1 eq.
  destruct (beq_nat n 3).
  (* stuck... *)
Abort.
...
MProof.
  ...
  destruct_eq (beq_nat _ 3) &> intro Heqe3.
  (* Now we have the same state as at the point where we got
     ...
     make progress. *)
    - (* e3 = true *) simpl. T.apply_in (beq_nat_true _ _) Heqe3. (* FIX: not very useful apply_in tactic... *)
    ...
    - (* e3 = false *)
     (* When we come to the second equality test in the body
        ...
        proof. *)
      destruct_eq (beq_nat _ _) &> intro Heqe5.
        + (* e5 = true *)
          T.apply_in (beq_nat_true _ _) Heqe5.
          rewrite -> Heqe5. reflexivity.
        + (* e5 = false *) intro eq. inversion eq.  Qed.
