...
Example reflect_sort := Eval compute in let (sort, _) := get_reflect_ITele in sort.
...
Fixpoint get_ATele {isort} : forall (it : ITele isort), list dyn -> M (ATele it) :=
  fix rec it al :=
    match it as it', al return M (ATele it') with
    | iBase T, nil => ret aBase
    | iTele f, t_dyn :: al =>
      t <- coerce (elem t_dyn);
        r <- rec (f t) al;
        ret (aTele t r)
...
Example get_RTrue_CTele := Eval compute in ltac:(mrun (get_CTele_SType reflect_itele 1 _ (RTrue True))).
Example get_RFalse_CTele := Eval compute in ltac:(mrun (get_CTele_SType reflect_itele _ (RFalse True))).
