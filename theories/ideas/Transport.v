...
Definition get_ind_cts (A : Type) (offset : nat) : M (nat * {s : Sorts.Sort & { it : ITele s & mlist (NDCTele it)}}) :=
  ind <- get_ind A;
  ...
  cts <- M.map (fun c_dyn : dyn =>
    ...
    let ty_c := reduce (RedWhd RedAll) (Sorts.stype_of isort) in
    ty <- M.evar ty_c;
    b <- M.cumul UniMatchNoRed ty dtype;
    if b then
      let el_c := reduce (RedWhd RedAll) (Sorts.selem_of ty) in
      el <- M.evar el_c;
...
Definition stype_type (sort : Sorts.Sort) (t : Sorts.stype_of sort) : Type := (Sorts.selem_of t) : Type.

Definition gen_match_branch {T1 T2 X} (recursor : T1 -> M T2) (base : T2 -> M X) :=
  fix f (C1_types C2_types : mlist Type) :
    forall ndc : (NDCfold (@iBase Sorts.SType T1) C1_types), (lprod C2_types -> M X) -> M (branch_of_NDCTele (rsort := Sorts.SProp) (it := @iBase Sorts.SType T1) (fun _ => M X) (existT _ C1_types ndc)) :=
    ...
    | X1:m:C1_types, X2:m:C2_types =>
      mtmmatch (m: X1, X2) as Xs return
          forall ndc : (NDCfold (@iBase Sorts.SType T1) (mfst Xs:m:C1_types)),
            (lprod (msnd Xs:m:C2_types) -> M X) ->
            M (branch_of_NDCTele (rsort := Sorts.SProp) (it := @iBase Sorts.SType T1) (fun _ => M X) (existT _ (mfst Xs:m:C1_types) ndc))
        with
        | [? A : Type] (m: A, A) =u>
...
Definition gen_match_from_to (T1 T2 : Type) X (offset : nat) : M (forall recursor : T1 -> M T2, forall base : T2 -> M X, T1 -> M X) :=
  ...
  mmatch (m: i1, i2) with
  | [?(n1 : nat) (Cs1 : mlist (NDCTele (@iBase Sorts.SType T1)))
      (n2 : nat) (Cs2 : mlist (NDCTele (@iBase Sorts.SType T2)))]
      (m:
        ((n1, existT _ Sorts.SType (existT _ (@iBase Sorts.SType T1) Cs1))),
        ((n2, existT _ Sorts.SType (existT _ (@iBase Sorts.SType T2) Cs2)))
      ) =>
    let it1 := @iBase Sorts.SType T1 in
    let it2 := @iBase Sorts.SType T2 in
