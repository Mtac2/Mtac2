...
Fixpoint is_open (g : goal) : M bool :=
  ...
  | @AHyp C _ f =>
    ...
    M.nu M.Generate mNone (fun x : C => is_open (f x))
...
Definition open_and_apply {A} (t : gtactic A) : gtactic A :=
  fix open g :=
    ...
    | Goal _ _ => t g
    | @AHyp C mNone f =>
      M.nu (M.FreshFrom f) mNone (fun x : C =>
        open (f x) >>= close_goals x)
    | @AHyp C (mSome t) f =>
      M.nu (M.FreshFrom f) (mSome t) (fun x : C =>
...
Definition intro_base {A B} (var : M.name) (t : A -> gtactic B) : gtactic B := fun g =>
...
Definition intro_cont {A B} (t : A -> gtactic B) : gtactic B := fun g=>
  ...
  intro_base (M.TheName n) t g.

(** Given a name of a variable, it introduces it in the context *)
Definition intro_simpl (var : M.name) : tactic := fun g =>
  A <- M.evar Type;
...
Definition intro_anonymous {A} (T : A) (g : goal) : M goal :=
  res <- intro_simpl (M.FreshFrom T) g >>= M.hd;
  M.ret (msnd res).
...
Definition copy_ctx {A} (B : A -> Type) : dyn -> M Type :=
  mfix1 rec (d : dyn) : M Type :=
    ...
    | [? C (D : C -> Type) (c : forall y:C, D y)] Dyn c =>
      M.nu (M.FreshFrom c) mNone (fun y=>
        r <- rec (Dyn (c y));
        M.abs_prod_type y r)
    | [? C D (c : C->D)] Dyn c =>
      M.nu (M.FreshFrom c) mNone (fun y=>
...
Definition destructn (n : nat) : tactic :=
  bind (introsn n) (fun _ g =>
    A <- M.evar Type;
    @intro_base A _ (M.FreshFrom "tmp"%string) destruct g).
...
Definition change_hyp {P Q} (H : P) (newH: Q) : tactic := fun g=>
  match g with
  | @Goal sort gT _ =>
     ''(m: gabs, abs) <- M.remove H (M.nu (M.FreshFrom H) mNone (fun nH: Q=>
       r <- M.evar gT;
       abs <- M.abs_fun nH r;
...
Definition cassert_with_base {A B} (name : M.name) (t : A)
...
Definition cpose_base {A B} (name : M.name) (t : A)
    (cont : A -> gtactic B) : gtactic B := fun g =>
  M.nu name (mSome t) (fun x=>
    match g with
    | @Goal sort gT _ =>
...
Definition cpose {A} (t: A) (cont : A -> tactic) : tactic := fun g =>
  cpose_base (M.FreshFrom cont) t cont g.

(* FIX: seriously need to abstract these set of functions!
   Too much duplication! *)
Definition cassert_base {A} (name : M.name)
...
Definition cassert {A} (cont : A -> tactic) : tactic := fun g=>
  cassert_base (M.FreshFrom cont) cont g.
  ...
  (fix loop (n : nat) (d : dynr) : M (typer d) :=
    ...
    | S n' =>
       ...
       | [? B (T:B->Type) f] @Dynr (forall x:B, T x) f =>
         ...
         M.nu (M.FreshFrom ty) mNone (fun x:B =>
...
(** [fix_tac f n] is like Coq's [fix] tactic: it generates a fixpoint
    ...
    [n] products. *)
Definition fix_tac (f : M.name) (n : N) : tactic := fun g =>
  gT <- M.goal_type g;
...
Definition map_term (f : forall d:dynr, M d.(typer)) : forall d : dynr, M d.(typer) :=
  mfix1 rec (d : dynr) : M d.(typer) :=
    ...
    | [? B (A: B -> Type) (a: forall x, A x)] Dynr (fun x:B=>a x) =n>
      M.nu (M.FreshFrom el) mNone (fun x : B =>
        d1 <- rec (Dynr (a x));
        M.abs_fun x d1)
...
Fixpoint intros_simpl (l : list string) : tactic :=
  match l with
  | nil => idtac
  | n :: l => bind (intro_simpl (M.TheName n)) (fun _ => intros_simpl l)
  end%list.
