...
Definition change_hyp {P Q} (H : P) (newH: Q) : tactic := fun g=>
  ...
  ''(m: gabs, abs) <- M.remove H (M.nu name mNone (fun nH: Q=>
     ...
     M.ret (m: AHyp SType mNone gabs, abs)));
  exact (abs newH) g;;
  M.ret [m:(m: tt, gabs)].
...
Definition fix_tac (f : string) (n : N) : tactic := fun g =>
  ...
  ''(f, new_goal) <- M.nu f mNone (fun f : gT =>
    ...
    M.ret (fixp, AHyp SType mNone new_goal)
  );
  exact f g;;
...
Definition selectT (T : Type) : gtactic T := fun g=>
  match g with
  | @Goal SProp A _ => (match_goal with [[ x : T |- A ]] => T.ret x end) g
  | @Goal SType A _ => (match_goal with [[ x : T |- A ]] => T.ret x end) g
  | _ => M.raise NotAGoal
  end.

Definition selectP (T : Prop) : gtactic T := fun g=>
  match g with
  | @Goal SProp A _ => (match_goal with [[ x : T |- A ]] => T.ret x end) g
  | @Goal SType A _ => (match_goal with [[ x : T |- A ]] => T.ret x end) g
  | _ => M.raise NotAGoal
