...
let raw_whd_state_gen flags env =
  let f sigma s = fst (whd_state_gen ~refold:(get_refolding_in_reduction ())
                         ~tactic_mode:false
                         flags env Environ.empty_env sigma s) in
  f
...
let iterate_whd_gen refold flags env sigma s =
  let rec aux t =
    let (hd,sk),_ = whd_state_gen ~refold ~tactic_mode:false flags env Environ.empty_env sigma (t,Stack.empty) in
...
let whd_betaiota_deltazeta_for_iota_state ts env sigma csts s =
  ...
  let rec whrec csts s =
    let (t, stack as s),csts' = whd_state_gen ~csts ~refold ~tactic_mode CClosure.betaiota env Environ.empty_env sigma s in
    match Stack.strip_app stack with
    |args, (Stack.Case _ :: _ as stack') ->
        let (t_o,stack_o),csts_o = whd_state_gen ~csts:csts' ~refold ~tactic_mode
                                     (CClosure.RedFlags.red_add_transparent CClosure.all ts) env Environ.empty_env sigma (t,args) in
