...
let inReductionEffect : Constr.constr * string -> obj =
  declare_object {(default_object "MACHINE-REDUCTION-EFFECT") with
...
module ReductionBehaviour = struct
  ...
  let table =
    Summary.ref (Refmap.empty : t Refmap.t) ~name:"machinereductionbehaviour"
  ...
  let inRedBehaviour = declare_object {
    (default_object "MACHINEREDUCTIONBEHAVIOUR") with
    load_function = load;
    ...
    | Lambda (na,t,c) ->
        (match Stack.decomp stack with
         ...
         | None when CClosure.RedFlags.red_set flags CClosure.RedFlags.fETA ->
             ...
             let whrec' = whd_state_gen ~refold ~tactic_mode flags env' fixs sigma in
...
let raw_whd_state_gen flags env =
  let f sigma s = fst (whd_state_gen ~refold:(get_refolding_in_reduction ())
                         ~tactic_mode:false
                         flags env Context.Named.empty sigma s) in
  f
...
let iterate_whd_gen refold flags env sigma s =
  let rec aux t =
    let (hd,sk),_ = whd_state_gen ~refold ~tactic_mode:false flags env Context.Named.empty sigma (t,Stack.empty) in
...
(*                         Conversion                               *)
(********************************************************************)
(* let fkey = Profile.declare_profile "fhnf";; let fhnf info v =
   Profile.profile2 fkey fhnf info v;;
...
let whd_betaiota_deltazeta_for_iota_state ts env sigma csts s =
  ...
  let rec whrec csts s =
    let (t, stack as s),csts' = whd_state_gen ~csts ~refold ~tactic_mode CClosure.betaiota env Context.Named.empty sigma s in
    match Stack.strip_app stack with
    |args, (Stack.Case _ :: _ as stack') ->
        let (t_o,stack_o),csts_o = whd_state_gen ~csts:csts' ~refold ~tactic_mode
                                     (CClosure.RedFlags.red_add_transparent CClosure.all ts) env Context.Named.empty sigma (t,args) in
