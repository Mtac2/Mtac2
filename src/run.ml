...
let rec run' ctxt t =
  ...
  if isLetIn sigma h then
    ...
    | 23 -> (* pretty_print *)
        let t = nth 1 in
        let s = string_of_ppcmds (Termops.print_constr_env env sigma t) in
...
let run (env, sigma) t =
  let subs, env = db_to_named sigma env in
  ...
  | Err (sigma', v) ->
      Err (sigma', multi_subst_inv sigma' subs v)
  | Val (sigma', v) ->
      Val (sigma', multi_subst_inv sigma' subs v)
